...
//    
//    CGSize contentSize = [content boundingRectWithSize:CGSizeMake(frame.size.width, 100) options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading   attributes:attribute context:nil].size;
    ...
    CGSize contentSize = [content boundingRectWithSize:CGSizeMake(frame.size.width, 100) // 用于计算文本绘制时占据的矩形块
                                                  options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading // 文本绘制时的附加选项
                                               attributes:dic        // 文字的属性
                                                  context:nil].size; // context上下文。包括一些信息，例如如何调整字间距以及缩放。该对象包含的信息将用于文本绘制。该参数可为nil
    NSLog(@"w = %f", contentSize.width);
    NSLog(@"h = %f", contentSize.height);
